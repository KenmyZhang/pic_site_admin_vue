<template>
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="排序">
          <el-input v-model.number="form.sort_order" style="width: 370px;" />
        </el-form-item>
        <el-form-item style="margin-bottom: 0;" label="上级分类" prop="parent_id">
          <treeselect v-model="form.parent_id" :options="depts" :normalizer="normalizer" style="width: 370px;" placeholder="选择上级分类" />
        </el-form-item>
      </el-form>
    ...
    <el-table ref="table" v-loading="crud.loading" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="columns.visible('sort_order')" label="排序" prop="sort_order" sortable/>
...
<script>
import crudDept from '@/api/shop/StoreCategory'
...
const defaultForm = { id: null, name: null, parent_id: 0, is_show: 1 , sort_order:  1}
export default {
  name: 'Cate',
  ...
  watch: {
    picArr: function(val) {
      console.log();
      this.form.wap_banner_url = val.join(',')
    }
  },
  methods: {
    // 新增与编辑前做的操作
    [CRUD.HOOK.afterToCU](crud, form) {
      this.picArr = []
      if (form.wap_banner_url && form.id) {
        this.picArr = form.wap_banner_url.split(',')
      }
      // 获取所有部门
