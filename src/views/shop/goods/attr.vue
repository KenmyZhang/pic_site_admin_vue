        ...
        <template v-for="(attr,index) in attrs">
          <el-form-item>
            <el-row :gutter="24">
              ...
              <el-col :span="4">
                <span :class="attr.check ? 'check':0">金额:</span>&nbsp;
                <el-input v-model="attr.price" placeholder="金额" style="width: 60%" :number="true" />
              </el-col>
              <el-col :span="4">
                <span :class="attr.check ? 'check':0">库存:</span>&nbsp;
                <el-input v-model="attr.stock" placeholder="库存" style="width: 60%" :number="true" />
              </el-col>
              <el-col :span="5">
                <span :class="attr.check ? 'check':0">成本价:</span>&nbsp;
                <el-input v-model="attr.cost" placeholder="成本价" style="width: 60%" :number="true" />
              </el-col>
              <el-col :span="3" style="margin-right: 2px">
                <div class="demo-upload">
                  <!--<img :src="attr.pic">-->
                  <pic-upload-two v-model="attr.image" />
                </div>
              </el-col>
...
import { add, edit, isFormatAttr, setAttr, clearAttr, getAttr } from '@/api/shop/StoreProduct'
...
export default {
  ...
  methods: {
    ...
    setAttrPic(index, image) {
      this.$set(this.attrs[index], 'image', image)
    },
    attrHiddenBool(item) {
      if (item.value == '') {
    ...
    checkGoods() {
      ...
      this.attrs.map(function(attr) {
        ...
        if (!Object.keys(attr.detail).length) {
          ...
          bool = false
        } else if (attr.price != attr.price || attr.price < 0) {
          Message({ message: '请输入正确的商品价格', type: 'error' })
          bool = false
        } else if (attr.sales != attr.sales || attr.sales < 0) {
